...
export class AlpineVersioningApi extends GenericVersioningApi<AlpineVersion> {
  protected _parse(version: string): AlpineVersion | null {
    ...
    const { digits, letter, suffix, suffixNumber, commitHash, revisionNumber } = matches;

    return {
      release: digits ? digits.split('.').map((num) => Number.parseInt(num, 10)) : [],
      letter: letter || undefined, 
      suffix: suffix || undefined,
      suffixNumber: suffixNumber ? Number.parseInt(suffixNumber, 10) : undefined,
      commitHash: commitHash || undefined,
      revisionNumber: revisionNumber ? Number.parseInt(revisionNumber, 10) : undefined
    };
  }

  protected override _compareOther(left: AlpineVersion, right: AlpineVersion): number {
