...
class ChainguardVersioningApi extends AlpineVersioningApi {
  protected override _parse(tag: string): ChainguardVersion | null {
    ...
    const date = parts[parts.length - 1].match(datePattern) ? parts.pop() : undefined;
    const variant = parts[parts.length - 1].match(variantPattern) ? parts.pop() : undefined;
  ...
  protected override _compareOther(left: ChainguardVersion, right: ChainguardVersion): number {
